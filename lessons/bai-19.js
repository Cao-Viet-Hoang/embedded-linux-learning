...
Lesson.register({
  ...
  blocks: [
    ...
    { t: 'fig',
      ...
      svg:
        ...
        '<text class="d-t" x="70" y="80">chep.c</text>' +
    ...
    { t: 'fig',
      ...
      svg:
        ...
        '<text class="d-ts" x="16" y="215">Đo trên máy của bạn: vòng này mất 140–320 ns, còn một lời gọi hàm thường chỉ mất 2–5 ns.</text>' +
        '</svg>' },
    ...
    { t: 'cal', kind: 'info', title: 'ARM64 bỏ hẳn open — và đó là lý do strace hiện openat', x:
      '<p>Khi bạn viết <code>open("goc.txt", O_RDONLY)</code>, glibc trên máy x86-64 hiện đại ' +
      '<b>không</b> gọi syscall số 2. Nó gọi <code>openat(AT_FDCWD, "goc.txt", O_RDONLY)</code> — ' +
    ...
    { t: 'code', where: 'file', name: 'tho.c', lang: 'c', code:
      '#include <sys/syscall.h>\n' +
      '#include <unistd.h>\n' +
      '\n' +
      'int main(void)\n' +
      '{\n' +
      '    const char *s = "Xin chao tu syscall tho\\n";\n' +
      '    syscall(SYS_write, 1, s, 24);\n' +
      '    syscall(SYS_exit, 7);\n' +
      '    return 0;   /* khong bao gio toi day */\n' +
      '}' },

    { t: 'code', where: 'wsl', code: 'gcc -Wall -o tho tho.c\n./tho\necho "exit=$?"' },

    { t: 'code', where: 'out', nocopy: true, code:
      'Xin chao tu syscall tho\nexit=7' },
    ...
    { t: 'cal', kind: 'why', title: 'Vì sao chuyển hướng của shell hoạt động được', x:
      '<p>Khi bạn gõ <code>./chep &gt; ket_qua.txt</code>, shell <b>mở file trước</b>, rồi ép ' +
    ...
    { t: 'code', where: 'file', name: 'fd.c', lang: 'c', code:
      ...
      'int main(void)\n' +
      '{\n' +
      '    printf("mo lan 1 -> fd=%d\\n", open("goc.txt", O_RDONLY));\n' +
      '    printf("mo lan 2 -> fd=%d\\n", open("goc.txt", O_RDONLY));\n' +
      '    printf("mo lan 3 -> fd=%d\\n", open("goc.txt", O_RDONLY));\n' +
      '    close(4);\n' +
      '    printf("dong fd 4, mo lai -> fd=%d\\n", open("goc.txt", O_RDONLY));\n' +
      '    return 0;\n' +
      '}' },
    ...
    { t: 'code', where: 'out', nocopy: true, code:
      'total 0\n' +
      'lrwx------ 1 shinarus shinarus 64 Aug  3 20:36 0 -> /dev/pts/2\n' +
      'lrwx------ 1 shinarus shinarus 64 Aug  3 20:36 1 -> /dev/pts/2\n' +
      'lrwx------ 1 shinarus shinarus 64 Aug  3 20:36 2 -> /dev/pts/2\n' +
      'lr-x------ 1 shinarus shinarus 64 Aug  3 20:36 3 -> /proc/460/fd' },
    ...
    { t: 'cal', kind: 'tip', title: 'Vì sao lại có fd 3 mà bạn không mở', x:
      ...
      '<code>/proc/460/fd</code> chứ không phải <code>/proc/self/fd</code>, vì nhân đã phân giải ' +
      '<code>self</code> thành số PID thật của lệnh <code>ls</code> — máy bạn sẽ ra số khác.</p>' +
      '<p>Nếu bạn chuyển hướng ra file — <code>ls -l /proc/self/fd &gt; ra.txt</code> — thì dòng 1 ' +
      'sẽ trỏ tới <code>ra.txt</code> thay vì <code>/dev/pts/1</code>. Đây là cách nhanh nhất để ' +
      'biết một tiến trình treo đang mở những gì trên thiết bị thật.</p>' },
    ...
    { t: 'cmdx', cmd: 'open("nhat_ky.txt", O_WRONLY | O_CREAT | O_APPEND, 0644)',
    ...
    { t: 'code', where: 'file', name: 'loi.c', lang: 'c', code:
      '#include <fcntl.h>\n' +
      '#include <unistd.h>\n' +
      '#include <stdio.h>\n' +
      '#include <errno.h>\n' +
      ...
      '    int fd;\n' +
      '\n' +
      '    fd = open("/khong/co/duong/nay", O_RDONLY);\n' +
      '    printf("1) tra ve=%d errno=%d %s\\n", fd, errno, strerror(errno));\n' +
      '\n' +
      '    fd = open("/etc/shadow", O_RDONLY);\n' +
      '    printf("2) tra ve=%d errno=%d %s\\n", fd, errno, strerror(errno));\n' +
      '\n' +
      '    fd = open("/etc", O_WRONLY);\n' +
      '    printf("3) tra ve=%d errno=%d %s\\n", fd, errno, strerror(errno));\n' +
      '\n' +
      '    /* errno KHONG duoc xoa khi thanh cong */\n' +
      '    fd = open("/etc/hostname", O_RDONLY);\n' +
      '    printf("4) tra ve=%d errno=%d %s  <- thanh cong nhung errno van con\\n",\n' +
      '           fd, errno, strerror(errno));\n' +
      '\n' +
      '    errno = 0;\n' +
      '    if (close(999) < 0)\n' +
    ...
    { t: 'code', where: 'out', nocopy: true, code:
      '1) tra ve=-1 errno=2 No such file or directory\n' +
      '2) tra ve=-1 errno=13 Permission denied\n' +
      '3) tra ve=-1 errno=21 Is a directory\n' +
      '4) tra ve=3 errno=21 Is a directory  <- thanh cong nhung errno van con\n' +
    ...
    { t: 'code', where: 'file', name: 'chiphi.c', lang: 'c', code:
      '#include <stdio.h>\n' +
      '#include <time.h>\n' +
      '#include <unistd.h>\n' +
      '#include <sys/syscall.h>\n' +
      ...
      '#define N 1000000\n' +
      '\n' +
      'static long ham_userspace(long x) { return x + 1; }\n' +
      '\n' +
      'int main(void)\n' +
      '{\n' +
      '    struct timespec t1, t2;\n' +
      '    volatile long tong = 0;\n' +
      '\n' +
      '    clock_gettime(CLOCK_MONOTONIC, &t1);\n' +
      '    for (int i = 0; i < N; i++) tong += ham_userspace(i);\n' +
      '    clock_gettime(CLOCK_MONOTONIC, &t2);\n' +
      '    double a = (t2.tv_sec - t1.tv_sec) * 1e9 + (t2.tv_nsec - t1.tv_nsec);\n' +
      '    printf("goi ham thuong  : %8.1f ns/lan\\n", a / N);\n' +
      '\n' +
      '    clock_gettime(CLOCK_MONOTONIC, &t1);\n' +
      '    for (int i = 0; i < N; i++) tong += syscall(SYS_getpid);\n' +
      '    clock_gettime(CLOCK_MONOTONIC, &t2);\n' +
      '    double b = (t2.tv_sec - t1.tv_sec) * 1e9 + (t2.tv_nsec - t1.tv_nsec);\n' +
      '    printf("goi syscall     : %8.1f ns/lan\\n", b / N);\n' +
      '    printf("cham hon        : %8.1f lan\\n", b / a);\n' +
      '    return 0;\n' +
      '}' },

    { t: 'cmdx', cmd: 'volatile long tong = 0;', title: 'Ba chi tiết bắt buộc của một phép đo tử tế',
      rows: [
        ['<code>volatile</code>', 'Cấm trình biên dịch bỏ qua vòng lặp', 'Không có nó, <code>-O2</code> thấy <code>tong</code> không được dùng và xoá sạch vòng lặp — bạn sẽ đo được 0 ns'],
    ...
    { t: 'code', where: 'wsl', code: 'gcc -Wall -O2 -o chiphi chiphi.c\n./chiphi' },

    { t: 'code', where: 'out', nocopy: true, code:
      'goi ham thuong  :      2.2 ns/lan\n' +
      'goi syscall     :    215.6 ns/lan\n' +
      'cham hon        :     97.3 lan' },

    { t: 'cal', kind: 'warn', title: 'Chạy nhiều lần — con số này dao động rất mạnh', x:
      '<p>Chín lần đo trên máy bạn cho: lời gọi hàm <b>1,6–5,4 ns</b>, syscall <b>139–317 ns</b>, ' +
      'tỉ lệ <b>43–191 lần</b>. Đừng trích một con số duy nhất rồi coi đó là chân lý.</p>' +
    ...
    /* ══════════════════════════════════════════════
       ...
       ══════════════════════════════════════════════ */
    { t: 'h2', x: 'Syscall thuần và stdio có đệm — cùng kết quả, khác 283 lần số syscall' },
    ...
    { t: 'fig',
      cap: 'stdio đổi 200 000 lần vượt biên giới lấy 706 lần — cùng một kết quả trên đĩa, nhưng nhanh hơn 6,3 lần.',
      svg:
        ...
        '<text class="d-ts" x="586" y="52">real 0,114 s</text>' +
        ...
        '<text class="d-t" x="406" y="172">706 syscall write</text>' +
        ...
        '<text class="d-ts" x="20" y="216">real 0,018 s   ·   sys 0,008 s   ·   hai file giống nhau từng byte (cmp không báo gì)</text>' +
        '<text class="d-ts" x="20" y="248">2 888 890 byte ÷ 4096 = 705,3 → 706 lần ghi. Con số 706 không phải ngẫu nhiên.</text>' +
        '</svg>' },

    { t: 'table',
      head: ['', '<code>write()</code> từng dòng', '<code>fprintf()</code> qua stdio'],
      rows: [
        ['Thời gian thực (tốt nhất trong 3 lần)', '<b>0,114 s</b>', '<b>0,018 s</b>'],
        ['Thời gian trong nhân (<code>sys</code>)', '0,095 s', '0,008 s'],
        ['Số lần gọi <code>write</code>', '<b>200 000</b>', '<b>706</b>'],
        ['Kích thước file kết quả', '2 888 890 byte', '2 888 890 byte'],
        ['Kết luận', '', '<b>nhanh hơn 6,3 lần</b>, ít hơn <b>283 lần</b> số syscall']
      ]},
    ...
    { t: 'cal', kind: 'info', title: 'Ba chế độ đệm của stdio — và vì sao chúng làm bạn bối rối', x:
      ...
      '</ul>' +
      '<p>Hệ quả bạn sẽ gặp ngay ở phần thực hành: chạy <code>./loi</code> trong terminal thì ' +
      'năm dòng ra đúng thứ tự 1→5; nhưng chạy <code>./loi &gt; ra.txt 2&gt;&amp;1</code> thì ' +
    ...
    { t: 'cmdx', cmd: 'strace -e trace=openat,read,write,close -o vet.txt ./chep goc.txt ban_sao.txt',
      ...
      rows: [
        ...
        ['<code>-o vet.txt</code>', 'Ghi vết ra file thay vì màn hình', '<b>Bắt buộc</b> khi chương trình cũng in ra màn hình, nếu không hai luồng trộn vào nhau'],
    ...
    { t: 'code', where: 'wsl',
      notes: ['Luôn dùng <code>-o</code> khi chương trình cũng in ra màn hình. Nếu không, dòng <code>fd nguon=3 fd dich=4</code> của <code>chep</code> sẽ chen ngang vào giữa một dòng vết của <code>strace</code>.'],
      code: 'strace -e trace=openat,read,write,close -o vet.txt ./chep goc.txt ban_sao.txt\ncat vet.txt' },
    ...
    { t: 'code', where: 'out', nocopy: true, code:
      ...
      'close(3)                                = 0\n' +
      'openat(AT_FDCWD, "goc.txt", O_RDONLY)   = 3\n' +
      'openat(AT_FDCWD, "ban_sao.txt", O_WRONLY|O_CREAT|O_TRUNC, 0644) = 4\n' +
      'read(3, "dong 1\\ndong 2\\ndong 3\\n", 4096) = 21\n' +
      'write(4, "dong 1\\ndong 2\\ndong 3\\n", 21) = 21\n' +
      'read(3, "", 4096)                       = 0\n' +
      'close(3)                                = 0\n' +
      'close(4)                                = 0\n' +
      'write(1, "fd nguon=3  fd dich=4\\n", 22) = 22\n' +
      '+++ exited with 0 +++' },
    ...
    { t: 'list', ordered: true, items: [
      ...
      '<code>read(3, ..., 4096) = 21</code>: yêu cầu 4096 byte, nhận về <b>21</b>. Đúng bằng kích thước <code>goc.txt</code>. Đây là <i>đọc thiếu</i> mà không phải lỗi.',
      ...
      'fd của <code>goc.txt</code> là <b>3</b>, đúng như dự đoán: 0/1/2 đã bị chiếm, 3 là số nhỏ nhất còn trống. Sau khi <code>libc.so.6</code> đóng fd 3, số đó được tái sử dụng.',
    ...
    { t: 'cal', kind: 'tip', title: 'strace -c: câu hỏi "chương trình này gọi gì nhiều nhất?"', x:
      '<p>Với <code>./chep</code>, tổng kết là <b>42 syscall, 2 lỗi</b> — trong đó chỉ 9 lời gọi ' +
    ...
    { t: 'cal', kind: 'warn', title: 'strace làm chương trình chậm đi hàng chục lần', x:
      ...
      '<code>write</code> được báo là <b>4,63 giây</b>, trong khi chạy không có <code>strace</code> ' +
      'thì cả chương trình chỉ mất <b>0,114 giây</b> — chênh <b>40 lần</b>.</p>' +
    ...
    { t: 'steps', items: [
      ...
      /* ---------- BƯỚC 2 ---------- */
      { title: 'Viết chep.c — lệnh cp tối giản',
        blocks: [
          { t: 'p', x:
          ...
          { t: 'code', where: 'file', name: 'chep.c', lang: 'c', code:
            '#include <fcntl.h>\n' +
            '#include <unistd.h>\n' +
            '#include <stdio.h>\n' +
            '#include <errno.h>\n' +
            ...
            '\n' +
            '    if (argc != 3) {\n' +
            '        fprintf(stderr, "Dung: %s <nguon> <dich>\\n", argv[0]);\n' +
            '        return 1;\n' +
            '    }\n' +
            '\n' +
            '    nguon = open(argv[1], O_RDONLY);\n' +
            '    if (nguon < 0) {\n' +
            ...
            '    }\n' +
            '\n' +
            '    dich = open(argv[2], O_WRONLY | O_CREAT | O_TRUNC, 0644);\n' +
            '    if (dich < 0) {\n' +
            '        fprintf(stderr, "open %s: %s (errno=%d)\\n",\n' +
            ...
            '    }\n' +
            '\n' +
            '    while ((n = read(nguon, dem, sizeof dem)) > 0) {\n' +
            '        ssize_t da_ghi = 0;\n' +
            '        while (da_ghi < n) {                       /* write co the ghi thieu */\n' +
            '            ssize_t m = write(dich, dem + da_ghi, n - da_ghi);\n' +
            '            if (m < 0) {\n' +
            '                perror("write");\n' +
            '                close(nguon); close(dich);\n' +
            '                return 1;\n' +
            '            }\n' +
            '            da_ghi += m;\n' +
            '        }\n' +
            '    }\n' +
            '    if (n < 0) {                                   /* n == 0 la het file */\n' +
            '        perror("read");\n' +
            '        close(nguon); close(dich);\n' +
            '        return 1;\n' +
            '    }\n' +
            '\n' +
            '    printf("fd nguon=%d  fd dich=%d\\n", nguon, dich);\n' +
            '    close(nguon);\n' +
            '    close(dich);\n' +
            '    return 0;\n' +
            '}' },
          ...
          { t: 'code', where: 'wsl', code:
            'gcc -Wall -Wextra -O2 -o chep chep.c\n' +
            'printf \'dong 1\\ndong 2\\ndong 3\\n\' > goc.txt\n' +
            './chep goc.txt ban_sao.txt\n' +
            'cmp goc.txt ban_sao.txt && echo "GIONG NHAU TUNG BYTE"' },

          { t: 'code', where: 'out', nocopy: true, code:
            'fd nguon=3  fd dich=4\n' +
            'GIONG NHAU TUNG BYTE' },

          { t: 'cmdx', cmd: 'while ((n = read(nguon, dem, sizeof dem)) > 0)',
            title: 'Vì sao vòng lặp phải viết đúng như vậy',
            rows: [
              ['<code>sizeof dem</code>', 'Kích thước đệm, do trình biên dịch tính', 'Không viết cứng số <code>4096</code> ở hai chỗ — đổi một chỗ quên chỗ kia là tràn bộ đệm'],
      ...
      { title: 'Nhìn xuyên chương trình bằng strace',
        blocks: [
          ...
          { t: 'code', where: 'wsl', code:
            'strace -e trace=openat,read,write,close -o vet.txt ./chep goc.txt ban_sao.txt\n' +
            'tail -9 vet.txt' },
          ...
          { t: 'code', where: 'wsl',
            ...
            code: 'strace -c -S calls -U name,calls,errors -o tk.txt ./chep goc.txt ban_sao.txt\ncat tk.txt' },
          ...
          { t: 'cal', kind: 'why', title: '42 syscall cho một chương trình chép 21 byte', x:
            '<p>Chỉ <b>9</b> trong số đó là việc thật (2 <code>openat</code>, 2 <code>read</code>, ' +
            '2 <code>write</code>, 2 <code>close</code>, 1 <code>execve</code>). 33 lời gọi còn ' +
            ...
            '<p>Cột <code>errors</code> báo <b>2</b> lỗi — và đó là chuyện <i>bình thường</i>: ' +
            '<code>access("/etc/ld.so.preload")</code> trả về <code>ENOENT</code> vì file đó ' +
            'không tồn tại, và một <code>ioctl</code> thất bại vì <code>stdout</code> lúc này ' +
            'không phải terminal. Bài học: <b>thấy lỗi trong <code>strace</code> không có nghĩa ' +
            'là chương trình hỏng.</b> Rất nhiều lỗi là phép thử có chủ ý.</p>' +
            '<p>Con số 33-syscall-cố-định này chính là lý do Chặng 09 quan tâm tới liên kết tĩnh ' +
      ...
      { title: 'Ép lỗi để nhìn errno hoạt động',
        blocks: [
          { t: 'p', x:
            ...
            '<code>chep</code> vào hai tình huống lỗi khác nhau và xem nó có nói đúng nguyên nhân ' +
            'không:' },

          { t: 'code', where: 'wsl', code:
            './chep khong_co.txt ra.txt ; echo "exit=$?"\n' +
            './chep /etc/shadow ra.txt  ; echo "exit=$?"' },

          { t: 'code', where: 'out', nocopy: true, code:
            'open khong_co.txt: No such file or directory (errno=2)\n' +
            'exit=1\n' +
          ...
          { t: 'p', x:
            ...
            '<code>loi.c</code> ở phần lý thuyết, <b>hai lần</b> — một lần ra terminal, một lần ' +
            'chuyển hướng vào file:' },

          { t: 'code', where: 'wsl', code:
            'gcc -Wall -o loi loi.c\n' +
            './loi\n' +
      ...
      /* ---------- BƯỚC 5 ---------- */
      { title: 'Đo giá của đệm: 200 000 syscall so với 706',
        blocks: [
          { t: 'p', x:
          ...
          { t: 'code', where: 'file', name: 'dem.c', lang: 'c', code:
            ...
            '#include <fcntl.h>\n' +
            '\n' +
            '#define SO_DONG 200000\n' +
            '\n' +
            '/* write() co the ghi thieu -> luon lap cho den het */\n' +
            'static int ghi_het(int fd, const char *p, size_t n)\n' +
            '{\n' +
            '    while (n > 0) {\n' +
            ...
            '}\n' +
            '\n' +
            'static void cach_syscall(const char *ten)\n' +
            '{\n' +
            '    int fd = open(ten, O_WRONLY | O_CREAT | O_TRUNC, 0644);\n' +
            '    char dong[64];\n' +
            '    for (int i = 0; i < SO_DONG; i++) {\n' +
            '        int n = snprintf(dong, sizeof dong, "dong so %d\\n", i);\n' +
            '        ghi_het(fd, dong, n);\n' +
            '    }\n' +
            '    close(fd);\n' +
            '}\n' +
            '\n' +
            'static void cach_stdio(const char *ten)\n' +
            '{\n' +
            '    FILE *f = fopen(ten, "w");\n' +
            '    for (int i = 0; i < SO_DONG; i++)\n' +
            '        fprintf(f, "dong so %d\\n", i);\n' +
            ...
            '{\n' +
            '    if (argc != 3) {\n' +
            '        fprintf(stderr, "Dung: %s <syscall|stdio> <file>\\n", argv[0]);\n' +
            '        return 1;\n' +
            '    }\n' +
            '    if (strcmp(argv[1], "syscall") == 0) cach_syscall(argv[2]);\n' +
            '    else                                 cach_stdio(argv[2]);\n' +
            '    return 0;\n' +
            '}' },

          { t: 'code', where: 'wsl', code:
            'gcc -Wall -Wextra -O2 -o dem dem.c\n' +
            'TIMEFORMAT=\'real %3R  user %3U  sys %3S\'\n' +
            'for i in 1 2 3; do time ./dem syscall a.txt; done\n' +
            'for i in 1 2 3; do time ./dem stdio   b.txt; done\n' +
            'cmp a.txt b.txt && echo "HAI FILE GIONG HET NHAU"' },

          { t: 'code', where: 'out', nocopy: true, code:
            'real 0.119  user 0.025  sys 0.095\n' +
            'real 0.114  user 0.012  sys 0.100\n' +
            'real 0.115  user 0.037  sys 0.075\n' +
            'real 0.018  user 0.007  sys 0.008\n' +
            'real 0.020  user 0.015  sys 0.004\n' +
            'real 0.020  user 0.011  sys 0.004\n' +
          ...
          { t: 'code', where: 'wsl', code:
            'strace -c -e trace=write ./dem syscall a.txt 2>&1 | sed -n \'3p\'\n' +
            'strace -c -e trace=write ./dem stdio   b.txt 2>&1 | sed -n \'3p\'' },

          { t: 'code', where: 'out', nocopy: true, code:
            '100.00    4.634999          23    200000           write\n' +
            '100.00    0.008451          11       706           write' },

          { t: 'table',
            head: ['Chỉ số', '<code>write()</code> thuần', '<code>stdio</code>', 'Tỉ lệ'],
            rows: [
              ['Thời gian thực tốt nhất', '0,114 s', '0,018 s', '<b>6,3 lần</b>'],
              ['Thời gian trong nhân', '0,095 s', '0,008 s', '11,9 lần'],
              ['Số lần gọi <code>write</code>', '200 000', '706', '<b>283 lần</b>'],
              ['Byte mỗi lần gọi', '≈ 14', '4096', '—']
            ]},

          { t: 'cal', kind: 'info', title: 'Kiểm tra con số 706 — nó phải đúng bằng phép chia', x:
            '<p>File kết quả nặng <b>2 888 890</b> byte. Chia cho đệm 4096: <b>705,3</b>. Làm ' +
            'tròn lên vì phần dư cũng phải được ghi → <b>706</b>. Đúng khớp với ' +
            '<code>strace</code>.</p>' +
            ...
            'lần <code>write</code> phải tụt xuống còn 45.</p>' +
            '<p>Chú ý cột thời gian dưới <code>strace</code>: <b>4,63 s</b> cho 200 000 lời gọi. ' +
            'Chạy không có <code>strace</code> thì chỉ 0,114 s. <code>strace</code> làm chậm ' +
            '<b>40 lần</b> — dùng nó để đếm, đừng dùng để đo.</p>' }
        ]},
      ...
      { title: 'lseek: tạo file 10 MB chỉ tốn 8 KB đĩa',
        blocks: [
          ...
          { t: 'code', where: 'file', name: 'thua.c', lang: 'c', code:
            '#include <fcntl.h>\n' +
            '#include <unistd.h>\n' +
            '#include <stdio.h>\n' +
            '\n' +
            'int main(void)\n' +
            '{\n' +
            '    int fd = open("thua.bin", O_WRONLY | O_CREAT | O_TRUNC, 0644);\n' +
            '    off_t vt;\n' +
            '\n' +
            '    write(fd, "DAU", 3);\n' +
            '    vt = lseek(fd, 10 * 1024 * 1024, SEEK_SET);\n' +
            '    printf("sau lseek, vi tri = %lld\\n", (long long)vt);\n' +
            '    write(fd, "CUOI", 4);\n' +
            '\n' +
            '    vt = lseek(fd, 0, SEEK_CUR);\n' +
            '    printf("vi tri hien tai   = %lld\\n", (long long)vt);\n' +
            '    vt = lseek(fd, 0, SEEK_END);\n' +
            '    printf("kich thuoc file   = %lld\\n", (long long)vt);\n' +
            '    close(fd);\n' +
            '    return 0;\n' +
            '}' },

          { t: 'code', where: 'wsl', code:
            'gcc -Wall -o thua thua.c\n./thua\nls -l thua.bin\ndu -h thua.bin\ndu --apparent-size -h thua.bin' },

          { t: 'code', where: 'out', nocopy: true, code:
            'sau lseek, vi tri = 10485760\n' +
            'vi tri hien tai   = 10485764\n' +
            'kich thuoc file   = 10485764\n' +
            '-rw-r--r-- 1 shinarus shinarus 10485764 Aug  3 20:30 thua.bin\n' +
            '8.0K\tthua.bin\n' +
            '11M\tthua.bin' },
          ...
          { t: 'code', where: 'wsl', code:
            'xxd -s 0        -l 16 thua.bin\n' +
            'xxd -s 5242880  -l 16 thua.bin\n' +
            'xxd -s 10485760 -l 16 thua.bin' },

          { t: 'code', where: 'out', nocopy: true, code:
            '00000000: 4441 5500 0000 0000 0000 0000 0000 0000  DAU.............\n' +
            '00500000: 0000 0000 0000 0000 0000 0000 0000 0000  ................\n' +
            '00a00000: 4355 4f49                                CUOI' },
          ...
          { t: 'cal', kind: 'why', title: 'ls nói 10 MB, du nói 8 KB — ai đúng?', x:
            ...
            '<code>DAU</code> và <code>CUOI</code>.</p>' +
            '<p><b>Hệ quả thực tế ở Chặng 09.</b> File <code>rootfs.img</code> 2 GB có thể chỉ ' +
            'chiếm 300 MB đĩa. Nhưng <code>cat rootfs.img &gt; ban_sao.img</code> sẽ ghi thật cả ' +
      ...
      { title: 'Đọc thiết bị thật trong /dev và /sys',
        blocks: [
          ...
          { t: 'code', where: 'out', nocopy: true, code:
            'crw-rw-rw- 1 root root 1, 7 Aug  3 20:30 /dev/full\n' +
            'crw-rw-rw- 1 root root 1, 3 Aug  3 20:30 /dev/null\n' +
            'crw-rw-rw- 1 root root 1, 9 Aug  3 20:30 /dev/urandom\n' +
            'crw-rw-rw- 1 root root 1, 5 Aug  3 20:30 /dev/zero' },
          ...
          { t: 'code', where: 'wsl', code:
            ...
            'echo xin_chao > /dev/null ; echo "ghi vao /dev/null -> exit=$?"\n' +
            'echo xin_chao > /dev/full ; echo "ghi vao /dev/full -> exit=$?"' },

          { t: 'code', where: 'out', nocopy: true, code:
            '00000000: 8e84 3254 e576 0bbf                      ..2T.v..\n' +
            '00000000: 0000 0000 0000 0000 0000 0000 0000 0000  ................\n' +
            'ghi vao /dev/null -> exit=0\n' +
            'bash: echo: write error: No space left on device\n' +
            'ghi vao /dev/full -> exit=1',
          ...
          { t: 'cal', kind: 'tip', title: '/dev/full tồn tại để bạn thử được nhánh xử lý lỗi', x:
            ...
            '<p>Hãy thử ngay với chương trình bạn vừa viết — <code>./chep goc.txt /dev/full</code> ' +
            'phải cho ra:</p>' +
          ...
          { t: 'code', where: 'out', nocopy: true, code:
            ...
            '--w------- 1 root root 0 Aug  3 20:30 /proc/sys/vm/drop_caches' },
          ...
          { t: 'cal', kind: 'why', title: 'Đây chính là cách bạn sẽ bật một chiếc đèn LED', x:
            ...
            '— y hệt những gì bạn vừa viết trong <code>chep.c</code>. Không có API bí ẩn nào cả.</p>' +
    ...
    { t: 'table',
      ...
      rows: [
        ['<code>ignoring return value of \'write\' declared with attribute \'warn_unused_result\'</code>',
         'Gọi <code>write</code> mà không dùng giá trị trả về. Cảnh báo này gặp thật khi soạn <code>dem.c</code>',
         'Đừng dập bằng <code>(void)</code>. Viết hàm <code>ghi_het()</code> lặp cho tới khi ghi hết — như bản cuối trong bài'],
        ...
        ['Chương trình chạy đúng trên terminal, sai thứ tự khi <code>&gt; ra.txt</code>',
        ...
        ['Chương trình chậm hẳn khi bật <code>strace</code>',
         'Không phải lỗi — mỗi syscall bị chặn hai lần qua <code>ptrace</code>. Đo được <b>40 lần</b> chậm hơn trong bài',
    ...
    { t: 'recap', title: 'Tóm tắt Bài 19', items: [
      ...
      'Một syscall tốn <b>139–317 ns</b> trên máy bạn, so với <b>1,6–5,4 ns</b> cho một lời gọi hàm thường: đắt hơn khoảng <b>hai bậc thập phân</b>. Nguyên tắc: gọi ít lần, mỗi lần làm nhiều việc.',
      ...
      '<code>stdio</code> là lớp đệm <b>4096 byte</b> trong user space. Ghi 200 000 dòng: syscall thuần cần <b>200 000</b> lần <code>write</code>, <code>stdio</code> chỉ cần <b>706</b> — ít hơn <b>283 lần</b>, nhanh hơn <b>6,3 lần</b>, kết quả giống hệt từng byte.',
      ...
      '<code>strace</code> cho bạn thấy mọi thứ một chương trình làm với hệ thống, không cần mã nguồn — nhưng làm nó chậm khoảng <b>40 lần</b>.',
      '<code>/dev</code> và <code>/sys</code> biến phần cứng thành file. Bật một chiếc LED chính là <code>open</code> + <code>write</code> + <code>close</code> — không có gì khác so với <code>chep.c</code> bạn vừa viết.'
    ]},
  ...
  quiz: [
    ...
    {
      q: 'Đoạn mã sau sai ở đâu?<br><code>int n = read(fd, dem, 1000);<br>if (errno != 0) { perror("read"); return 1; }</code>',
      opts: [
      ...
      a: 1,
      why: 'Bạn đã thấy tận mắt trong <code>loi.c</code>: dòng 4 mở <code>/etc/hostname</code> <b>thành công</b>, trả về fd <b>3</b>, nhưng <code>errno</code> vẫn là <b>21</b> sót lại từ dòng 3. Chuẩn C chỉ bảo đảm <code>errno</code> được <i>đặt</i> khi lỗi, không bảo đảm được <i>dọn</i> khi thành công. Cách đúng là <code>if (n &lt; 0) { perror("read"); ... }</code>. Đáp án A đúng một phần — <code>int</code> thật sự sẽ tràn với file lớn — nhưng đó không phải lỗi nghiêm trọng nhất ở đây. Đáp án C chỉ vá tạm: nó không sửa được sai lầm gốc là dùng <code>errno</code> làm cờ báo lỗi.'
    },
    {
