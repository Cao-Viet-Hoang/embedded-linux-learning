...
| `bt-18` | `.bss` chiếm RAM lúc chạy nhưng **không** chiếm byte nào trong file — `MemSiz − FileSiz` đúng bằng nó · section và segment là **hai bản đồ độc lập** trên cùng dãy byte: một cho trình liên kết lúc build, một cho nhân lúc nạp · điểm vào là `_start`, không phải `main` — `main` chỉ là một tham số truyền cho `__libc_start_main` |
| `bt-19` | `write()` trả về **số byte thực sự ghi được** — nhỏ hơn số yêu cầu vẫn là thành công, không phải lỗi · `printf` ghi vào một **đệm nằm trong tiến trình**, nên "khi nào byte rời khỏi tiến trình" là một câu hỏi khác hẳn "byte đi đường nào" (ranh giới với `bt-10`) · `errno` chỉ có nghĩa **ngay sau** một lời gọi đã báo lỗi qua giá trị trả về; không lời gọi nào đặt nó về 0 khi thành công |
| `bt-20` | `fork()` trả về **hai lần**, mỗi lần một giá trị khác nhau — sau nó có hai tiến trình cùng chạy tiếp từ chính dòng đó, phân biệt nhau **chỉ bằng giá trị trả về** · `exec` **giữ nguyên PID** và thay toàn bộ ảnh bộ nhớ — tiến trình không chết, chỉ đổi nội dung; cũng vì thế mọi fd không `CLOEXEC` đi xuyên qua · từ trạng thái của `wait` là một **số nguyên đóng gói**: mã thoát ở byte cao, số hiệu tín hiệu ở 7 bit thấp — phải hỏi macro trước, không đọc thẳng |
...
| `bt-26` | Cái quyết định hai file có liên kết được với nhau không phải **kiến trúc** mà là **ABI** — cùng `aarch64` nhưng khác `gnueabihf`/`gnu`/`musl` thì trình liên kết vẫn từ chối, và bộ ba `arch-vendor-os-abi` là nơi đọc ra điều đó · công cụ **đọc vỏ ELF** (`readelf`, `nm`, `size`, `strings`) dùng chung được cho mọi kiến trúc, công cụ **giải mã lệnh hoặc chạy** (`objdump -d`, `gdb`, `strip`, `as`, `ld`) thì bắt buộc phải là bản có tiền tố đúng — đây là ranh giới giải thích mọi thông báo `Unable to recognise the architecture` · trình biên dịch cross **không tìm header và thư viện ở `/usr/include` của máy build** mà ở sysroot của target, nên thiếu một thư viện là chuyện của sysroot chứ không phải của `apt install` |
| `bt-27` | Chạy xanh dưới `qemu-user` chỉ chứng minh **mã lệnh đúng**, không chứng minh **môi trường đúng**: lớp mô phỏng mượn nhân, rootfs và thư viện của máy build, nên nó không thể phát hiện thiếu thư viện trên bo, sai `/etc`, hay sai thiết bị · nhị phân cross mang trong nó **đường dẫn của target, không phải của máy build** — `.interp` ghi `/lib/ld-linux-aarch64.so.1` theo hệ quy chiếu của bo, nên `No such file or directory` thường nói về *trình thông dịch*, không về file bạn vừa gõ tên · **file to hơn không có nghĩa là nhiều mã hơn** — 61 440 byte chênh lệch giữa hai bản ARM64 là đệm căn lề trang 64 KiB, và chỉ `size` (text/data/bss) mới trả lời được câu "có thêm mã không" |
